...
If your integration has multiple [flows](/composer/builder/building-integrations.mdx#flows-in-integrations), each flow has its own trigger (and so its own webhook URL or schedule).
...
Webhook triggers allow you to run a particular instance or [flow](/composer/builder/building-integrations.mdx#flows-in-integrations) of an instance by making an HTTP POST, PUT, PATCH, DELETE or GET request to the webhook's URL.
...
For information on configuring and troubleshooting webhook endpoints, see our [Endpoint Configuration](/composer/builder/endpoint-configuration.mdx) article.
...
- The url path - `my/custom/path` - is accessible through the trigger's `results.pathFragment`. You can pass that data into the built-in [split string](/composer/builder/builder-components/text-manipulation.mdx#split-string) action and split on the `/` character to split the URL path into an array `['my','custom','path']`.
...
A common pattern is for one [flow](/composer/builder/building-integrations.mdx#flows-in-integrations) of your integration to be run when an instance is deployed using a [deploy trigger](#deploy-trigger).
...
If you use [shared endpoint configuration](/composer/builder/endpoint-configuration.mdx), the shared endpoint URL is accessible from `results.invokeUrl`.

If you set up [API keys](/composer/builder/endpoint-configuration.mdx/#securing-endpoints-with-api-keys) for your deployed instances, you can access `results.webhookApiKeys` similarly.
...
If your HMAC hash is generated from the HTTP payload's body, the secret signing key is a string, and the hash is included as a header, you can use the built-in [HMAC Webhook Trigger](/composer/builder/builder-components/hash.mdx#hmac-webhook-trigger) to validate payloads.
...
You can read about how the Slack webhook trigger works in [this quickstart](/composer/builder/custom-components/quickstarts/custom-trigger-with-hmac.mdx).

## Instance deploy trigger

An integration flow can be configured to be run when an instance of the integration is [deployed](/composer/builder/instances.mdx#deploying-instances).
To do that, click your trigger and then click **CHANGE TRIGGER**.
Choose to replace your trigger with a [Management Triggers - Instance Deploy](/composer/builder/builder-components/management-triggers.mdx#instance-deploy) trigger.
...
The opposite of an [instance deploy trigger](#instance-deploy-trigger) is an [instance remove](/composer/builder/builder-components/management-triggers.mdx#instance-remove) trigger.
...
You can [build your own triggers](/composer/builder/custom-components/writing-custom-components.mdx#writing-triggers) as part of a custom component.
...
Most commonly, you can add a [Stop Execution](/composer/builder/builder-components/stop-execution.mdx) step to the end of your integration, and specify an HTTP response that it should return.
...
That execution ID can be exchanged later with the Builder API for logs and step results using the [executionResult](/composer/builder/api/schema/query/executionResult.mdx) GraphQL mutation.
...
Check out our [FIFO Queue](/composer/builder/custom-components/quickstarts/fifo-queue.mdx) tutorial for more information.
