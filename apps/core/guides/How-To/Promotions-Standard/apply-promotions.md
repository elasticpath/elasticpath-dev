...
- If a certain SKU or catalog is targeted in the [promotion schema](/docs/api/promotions/create-a-promotion#the-schema-object), a custom item will not be eligible for the promotion unless it is specifically included as a target.
:::
...
- Ensure that you have an [access token](/guides/Getting-Started/your-first-api-request#get-an-access-token).
...
1. Create a promotion using one of the following:
    ...
    - [Commerce Manager](/docs/commerce-manager/)
    - [API](/api)
...
1. [Add items to the cart](/docs/api/carts/manage-carts#add-product-to-cart).
1. Apply promotion using code if the promotion is not automatically applied.
1. [Checkout the cart](/docs/api/carts/checkout) that is updated with the promotion.
...
- [Pay for the order](/docs/api/carts/payments)
