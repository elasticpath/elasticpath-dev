...
- An **alert monitor** is a combination of an alert group and some alert triggers, and is configured for an [instance](/self-managed/composer/builder/instances.mdx).
...
[Log Levels](/self-managed/composer/builder/logging.mdx#log-levels)
...
- `$CUSTOMER_EXTERNAL_ID` - The [external ID](/self-managed/composer/builder/customers.mdx#customer-external-ids) of the customer the instance is deployed to
...
To create an alert webhook, use the [createAlertWebhook](/self-managed/composer/builder/api/schema/mutation/createAlertWebhook.mdx) mutation:
...
Delete an alert webhook by alert webhook ID using the [deleteAlertWebhook](/self-managed/composer/builder/api/schema/mutation/deleteAlertWebhook.mdx) mutation:
...
Once you have user IDs and alert webhook IDs, create an alert group using the [createAlertGroup](/self-managed/composer/builder/api/schema/mutation/createAlertGroup.mdx) mutation:
...
Delete an alert group using the [deleteAlertGroup](/self-managed/composer/builder/api/schema/mutation/deleteAlertGroup.mdx) mutation:
...
An **alert monitor** is a combination of an [alert group](#alert-groups) (users and webhooks) and an [alert trigger](#alert-triggers) that is configured for an [instance](/self-managed/composer/builder/instances.mdx).
...
Note that if your instances are configured to use a [shared endpoint](/self-managed/composer/builder/endpoint-configuration.mdx#shared-endpoint-configuration) and a **preprocess flow**, an alert monitor cannot be assigned to the preprocess flow since the preprocess flow runs independently of any deployed instance.
:::
...
Delete an alert monitor with the [deleteAlertMonitor](/self-managed/composer/builder/api/schema/mutation/deleteAlertMonitor.mdx) mutation:
...
List alert events for a particular alert monitor by querying [alertEvents](/self-managed/composer/builder/api/schema/query/alertEvents.mdx).
...
An [alert monitor](/self-managed/composer/builder/monitoring-and-alerting.mdx#alert-monitors) allows you to be notified when something happens in a particular instance's flow.
...
- A combination of `sortBy: { direction: ASC, field: CREATED_AT }`, `after: $cursor`, and `pageInfo` at the end of the query will allow us to [paginate](/self-managed/composer/builder/api/using-builder-api.mdx#pagination-with-prismatics-api) through the results.
...
Alert monitors can be [triggered](/self-managed/composer/builder/monitoring-and-alerting.mdx#alert-triggers) by a number of events, including when an execution fails, when an execution succeeds, or when an execution takes longer than a certain amount of time.
