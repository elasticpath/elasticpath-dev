...
An **instance** of an [integration](/self-managed/composer/builder/integrations.mdx) is a copy of an integration that has been configured for a specific [customer](/self-managed/composer/builder/customers.mdx).
When the instance is configured for a customer or by a customer, config variables and connections to third-party apps and services specific to that customer are set by walking through a [configuration wizard](/self-managed/composer/builder/configuration-wizard.mdx).

You can deploy instances of the integrations you create [on behalf of your customers](/self-managed/composer/builder/org-deployed-instances.mdx) or your customers can enable instances themselves through the [integration marketplace](/self-managed/composer/builder/integration-marketplace.mdx).
...
When a [flow](/self-managed/composer/builder/building-integrations.mdx#flows-in-integrations) within an instance is triggered, an [execution](/self-managed/composer/builder/executions.mdx) of your instance's flow runs.

## What happens when an instance is deployed

Once an instance is deployed, any triggers that are marked as [deploy triggers](/self-managed/composer/builder/integration-triggers.mdx#instance-deploy-trigger) are executed.
Note: the instance will still deploy even if a deploy trigger fails to run to completion.
We recommend adding an [alert monitor](/self-managed/composer/builder/monitoring-and-alerting.mdx#alert-monitors) to instances you deploy, so you can be alerted if an execution (a deploy flow included) fails to run to completion.
...
**More Information**:
[Webhook Triggers](/self-managed/composer/builder/integration-triggers.mdx#webhook-triggers)
...
As you work with your first few customers and build out an MVP, it may be easiest to [deploy instances yourself](/self-managed/composer/builder/org-deployed-instances.mdx).
...
You can do this by publishing your integration to the [integration marketplace](/self-managed/composer/builder/integration-marketplace.mdx) and then invite the [customer user](/self-managed/composer/builder/users.mdx#managing-customer-users) to your tenant.
...
You can [embed the integration marketplace](/self-managed/composer/builder/embedding-marketplace.mdx) in your app so that your customers can deploy instances without leaving your app.
...
If you want to build a custom UI for deploying instances, you can use the Builder embedded SDK to [make API requests](/self-managed/composer/builder/embedded-api-requests.mdx) to the Builder API on behalf of your customer user.
...
When you or your customer deploy an instance, you will work through the [Configuration Wizard](/self-managed/composer/builder/configuration-wizard.mdx) that your integration builders created.
