...
The ULC configuration wizard designer is very similar to the [configuration wizard designer](/self-managed/composer/builder/configuration-wizard.mdx#configuration-wizard-designer) - you can add connections and other types of config variables like you would in the normal configuration wizard.
...
When you [authenticate users in the embedded marketplace](/self-managed/composer/builder/installing-embedded.mdx#authenticating-users) you will need to include a `role` property in your signed JWT that has a value of either `"admin"` or `"user"`.
...
An admin user will [configure](/self-managed/composer/builder/instances.mdx#configuring-instances) a ULC instance like they would a non-ULC instance - by stepping through a configuration wizard to set up customer-wide configuration settings.
...
You have the same options for [endpoint configuration](/self-managed/composer/builder/endpoint-configuration.mdx) in ULC integrations that you do for non-ULC integrations, with the caveat that when you use shared endpoints you also need to specify a user by external user ID (like you do for flow name and external customer ID):
...
Like non-ULC integrations, ULC integrations with **Instance Specific** and **Shared** endpoint configuration can leverage a [preprocess flow](/self-managed/composer/builder/endpoint-configuration.mdx#shared-endpoint-without-a-preprocess-flow) to process incoming data and dispatch executions to the proper instance / user / flow.
