...
Integrations that use this endpoint configuration often set up webhooks with a [deploy trigger](/self-managed/composer/builder/integration-triggers.mdx#instance-deploy-trigger), and remove them with an [instance remove](/self-managed/composer/builder/builder-components/management-triggers.mdx#instance-remove) trigger.
...
If you do not use a preprocess flow, the shared endpoint's webhook invocation must include an [external customer ID](/self-managed/composer/builder/customers.mdx#customer-external-ids) and **flow name** either in the HTTP payload, or as HTTP headers.
...
The final step must return an object containing both a [external customer ID](/self-managed/composer/builder/customers.mdx#customer-external-ids) and a **flow name**.
...
An integration can have [multiple versions](/self-managed/composer/builder/integrations.mdx#publishing-an-integration), and customers' instance can be on different versions.
...
- **No API Keys** indicates that the flow can be invoked without an API key.
  This option is often paired with a trigger that handles security in another way (like with [HMAC](/self-managed/composer/builder/best-practices/secure-webhook-endpoints-hmac)).
...
If you have an integration with [Instance-Specific Endpoint Configuration](#instance-specific-endpoint-configuration), then all logs and execution records will appear in the instance's [execution results](/self-managed/composer/builder/instances.mdx#viewing-instance-execution-results) page.
