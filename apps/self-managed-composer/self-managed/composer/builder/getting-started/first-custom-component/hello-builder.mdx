...
Be sure that you've [set up your environment](/self-managed/composer/builder/getting-started/first-custom-component/set-up-environment.mdx) first.
...
These are all things that are covered fully in the [Writing Custom Components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx) article, but we can safely remove them for now.
...
Try to add a middle name input, but only print the middle initial, or add a title input that presents a [dropdown menu](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#dropdown-menu-inputs), so users can select "Mr.", "Mrs.", "Dr.", etc.
...
Once you're ready, head to the [next tutorial](/self-managed/composer/builder/custom-components/quickstarts/wrapping-an-api-in-a-component.mdx) where we'll cover making API calls from a custom component.
