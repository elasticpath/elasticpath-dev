...
If you compute an HMAC hash from the webhook's request body, your secret key is a string, and the hash is included as a header, you can use the built-in [HMAC Webhook Trigger](/self-managed/composer/builder/builder-components/hash.mdx#hmac-webhook-trigger) to validate webhook requests.
Most (but not all) HMAC implementations follow this pattern.

In this tutorial, we'll look at the code behind the [Slack Webhook Trigger](/self-managed/composer/builder/builder-components/slack.mdx#webhook), which differs from the generic HMAC trigger in two ways:
...
For more information on building custom components and custom triggers, check out the [Writing Custom Components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx) article.
