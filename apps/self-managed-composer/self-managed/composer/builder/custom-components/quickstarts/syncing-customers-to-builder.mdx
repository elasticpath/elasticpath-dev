...
When you embed Composer into your app, you need to [authenticate](/self-managed/composer/builder/installing-embedded.mdx#authenticating-users) your customer users as users of a specific customer in Composer.
...
In this tutorial, we'll cover how to sync customers from your app to Composer using Composer's [GraphQL API](/self-managed/composer/builder/api/api-overview.mdx).
...
To get an API key into our environment variables, we can leverage the `me:token` [prism subcommand](/self-managed/composer/builder/cli/prism.mdx#metoken):
...
To sync customers, we'll use the [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query and [createCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) and [deleteCustomer](/self-managed/composer/builder/api/schema/mutation/deleteCustomer.mdx) mutations.
...
This function runs a [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query against Composer's API.
...
```python
print(/self-managed/composer/builder/json.dumps(getCustomers()))
```
...
So, the [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query only returns 100 customers, even if we have more than 100 in the system.
...
The [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query allows you to filter customers by `externalId`.
...
```python
print(/self-managed/composer/builder/json.dumps(getCustomerByExternalId("abc-123")))
```
...
To do that, we'll use the the [createCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) GraphQL mutation.
...
To do that, we'll use the [deleteCustomer](/self-managed/composer/builder/api/schema/mutation/deleteCustomer.mdx) mutation.
