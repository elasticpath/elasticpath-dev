...
If the API you're creating a component for has an OpenAPI or WSDL spec, you can have `prism` [generate a component](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#custom-components-from-wsdls-or-openapi-specs) based off of your YAML, JSON, or XML spec file.
...
Like any new custom component, we can create our new component using a subcommand of [prism](/self-managed/composer/builder/cli/cli-usage.mdx):
...
At the top of the file, we'll import the `action` and `input` functions from our [custom component SDK](/self-managed/composer/builder/spectral/custom-component-library.mdx) library, and the `getAcmeErpClient` function we defined in `client.ts`:
...
Now that we've implemented our four actions for our component, let's add some [unit tests](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#unit-tests-for-custom-components) to verify that our actions return what we'd expect them to return.
