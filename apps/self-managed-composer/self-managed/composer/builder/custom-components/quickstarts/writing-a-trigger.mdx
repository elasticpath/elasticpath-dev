...
In this tutorial we'll walk through how we wrote a trigger for our [Salesforce component](/self-managed/composer/builder/builder-components/salesforce.mdx) to serve as an example for how you'd go about writing triggers for your own custom components.
...
However, integrations with Salesforce do not work well with the [standard webhook trigger](/self-managed/composer/builder/integration-triggers.mdx#webhook-triggers) for two reasons:

1. Salesforce outbound messages send XML payloads rather than JSON.
   This isn't a deal-breaker, but all integrations with Salesforce would then need to start with an extra [Deserialize XML](/self-managed/composer/builder/builder-components/change-data-format.mdx#deserialize-xml) step.
...
We could omit this step, but then every integration with Salesforce would require an extra [Deserialize XML](/self-managed/composer/builder/builder-components/change-data-format.mdx#deserialize-xml) step.
...
Similar to an [action](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#writing-actions), a trigger's `perform` function takes [context](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#the-context-parameter) and `params` (inputs) parameters (though this trigger doesn't happen to take inputs).
...
We can reference the raw body of the webhook payload, and ensure that it is cast to a string using the Spectral [toString](/self-managed/composer/builder/spectral/modules/_util_.md#const-tostring) function.
...
To read more about authoring triggers, check out our [Writing Custom Components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#writing-triggers) article.
