...
You can use the built-in [Amazon SQS](/self-managed/composer/builder/builder-components/aws-sqs.mdx) component to ensure that your integration processes requests one at a time.
...
1. A "setup" flow that creates and configures the SQS queue.
   The flow is triggered by an [instance deploy trigger](/self-managed/composer/builder/builder-components/management-triggers.mdx#instance-deploy) so that it runs when a customer deploys an instance.
   It contains a single action - [Create Queue](/self-managed/composer/builder/builder-components/aws-sqs.mdx#create-queue) - which is idempotent and can be run many times.
...
4. A "cleanup" flow that deletes the queue.
   This flow is triggered by an [instance delete trigger](/self-managed/composer/builder/builder-components/management-triggers.mdx#instance-delete) so that it runs when a customer deletes an instance.
...
- Use your own AWS account and credentials.
  You can provide default credentials in the config wizard designer, but mark the connection as [organization-visible](/self-managed/composer/builder/configuration-wizard.mdx#config-variable-visibility).
