...
The goal of this quickstart is to examine the [Amazon S3 component](/self-managed/composer/builder/builder-components/aws-s3.mdx) to get you accustomed to developing more advanced components that have multiple actions, handle credentials through connections, and handle non-string binary data input fields.
...
The [Slack component](/self-managed/composer/builder/builder-components/slack.mdx), for example, will only ever post string messages.
...
If a reference to binary data is passed in (for example, from an [HTTP GET](/self-managed/composer/builder/builder-components/http.mdx#httpget) action's output), `fileContents` will be an object with `.data` and `.contentType` properties.
