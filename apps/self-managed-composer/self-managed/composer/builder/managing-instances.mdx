...
When disabled, your instance will not execute on a cron schedule (if configured to use [scheduled triggers](/self-managed/composer/builder/integration-triggers.mdx#scheduled-triggers)), nor respond to webhook invocations.
...
Integrations can be configured to automatically [retry](/self-managed/composer/builder/building-integrations.mdx#integration-retry-configuration) in the event that an instance fails to run to completion.
...
To programmatically retry many failed executions, the [executionResults](/self-managed/composer/builder/api/schema/query/executionResults.mdx) query can be used to find executions that failed to run to completion.
You can then use the [replayExecution](/self-managed/composer/builder/api/schema/mutation/replayExecution.mdx) mutation to replay the execution.

For an example of how to bulk-replay failed executions, see our [examples repo](https://github.com/prismatic-io/examples/tree/main/api/replay-failed-executions) in GitHub or review the [Execution Retry & Replay](/self-managed/composer/builder/retry-and-replay.mdx) docs.
...
[Creating Alert Monitors](/self-managed/composer/builder/monitoring-and-alerting.mdx#creating-an-alert-monitor).
