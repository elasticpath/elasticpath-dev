...
The CLI tool is built using the [Composer API](/self-managed/composer/builder/api/api-overview.mdx), so any action that can be completed through the web app or API can be completed through the CLI tool, as well.
...
Prism's source code is available on [GitHub](https://github.com/prismatic-io/prism), and serves as a great example for how to wrap the [Composer API](/self-managed/composer/builder/api/api-overview.mdx).
...
See [Integrations in Multiple Regions](/self-managed/composer/builder/integrations-multiple-regions.mdx#logging-in-to-additional-regions-with-prism).
:::
...
A full list of `prism` commands can be found on the [Composer CLI Command Reference](/self-managed/composer/builder/cli/prism.mdx) page.
...
If you're working with [regions](/self-managed/composer/builder/integrations-multiple-regions.mdx) other than the default US commercial region, you can also specify an endpoint:
