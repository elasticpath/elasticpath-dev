...
The integrations that you build in the [integration designer](/self-managed/composer/builder/building-integrations.mdx) are meant to be reusable, and deployable across your hetrogenous customer base.
...
If your integration requires manual configuration of webhooks, the config wizard can also display the instance's webhook endpoints and API keys (see [Endpoint API keys in the config wizard](/self-managed/composer/builder/endpoint-configuration.mdx/#endpoint-api-keys-in-the-config-wizard)).
...
Config variables that you define in the config variable drawer can be used within your integration as inputs or steps, or through the [Branch](/self-managed/composer/builder/builder-components/branch.mdx) component to drive branching logic.
...
- **JSON Form** allows you to leverage [JSON Forms](https://jsonforms.io/) to build your users' configuration experience.
  The code backing JSON Form config variables are developed in [custom components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx) and return objects that are made up of key/value pairs.
...
If you're [writing a custom component](/self-managed/composer/builder/custom-components/writing-custom-components.mdx), note that you will need to cast your action's input to the correct format.
For example, you can `JSON.parse()` a JSON string, or run [`util.types.toNumber()`](/self-managed/composer/builder/spectral/modules/_util_.md#const-tonumber) or [`util.types.toBool()`](/self-managed/composer/builder/spectral/modules/_util_.md#const-tobool) on a number or boolean input. You can use a [`clean`](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#cleaning-inputs) function to simplify type casting.
:::
...
Under **Data Source**, select the component and data source that you would like to use (for example, the [Slack Select Channel](/self-managed/composer/builder/builder-components/slack.mdx#select-channel) data source).
...
See the full [JSON Forms](/self-managed/composer/builder/jsonforms.mdx) article for more information.
...
You can read more about connections and OAuth 2.0 on the [Connections](/self-managed/composer/builder/connections.mdx) article.
...
- **Embedded** makes it so the config variable does not show up in the config wizard, but your app is able to [set it programmatically](/self-managed/composer/builder/embedding-marketplace.mdx#dynamically-setting-config-variables-in-marketplace) through the embedded SDK.
