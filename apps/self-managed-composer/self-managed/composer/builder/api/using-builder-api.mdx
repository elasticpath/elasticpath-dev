...
Before you begin, please read through our [Intro to Builder's API](/self-managed/composer/builder/api/api-overview.mdx) article and generate an [API token](/self-managed/composer/builder/api/api-overview.mdx#how-do-i-get-an-api-authorization-token) for yourself, so you can make API calls.

:::tip Reference the prism CLI tool's source code
Builder's CLI tool, [prism](/self-managed/composer/builder/cli/cli-usage.mdx) is open-source and wraps the Builder API.
...
Let's start by managing [customers](/self-managed/composer/builder/customers.mdx) programmatically.
...
If you know the ID of the customer you would like to query, use the [customer](/self-managed/composer/builder/api/schema/query/customer.mdx) query.
...
If you would like information about several customers, use the [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query instead:
...
To add a new customer to your account, use the [createCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) _mutation_.
...
You can update a customer using the [updateCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) mutation.
...
The [deleteCustomer](/self-managed/composer/builder/api/schema/mutation/deleteCustomer.mdx) mutation takes a customer ID, and returns the customer (if it has been deleted), or an error if the customer can't be found.
...
If you're using the embedded marketplace and [signed JWTs](/self-managed/composer/builder/embedding-marketplace.mdx#authenticating-users) to seamlessly authenticate customer users, you probably won't need to manage your customer users programmatically.
...
First, look up the **roles** you can assign a user using either the [customerRoles](/self-managed/composer/builder/api/schema/query/customerRoles.mdx) or [organizationRoles](/self-managed/composer/builder/api/schema/query/organizationRoles.mdx) query:
...
Next, run the either the [createCustomerUser](/self-managed/composer/builder/api/schema/mutation/createCustomerUser.mdx) or [createOrganizationUser](/self-managed/composer/builder/api/schema/mutation/createOrganizationUser.mdx) mutation to create the user:
...
Once again we'll use the [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) query to list each customer's **ID** and **name**, but in addition we'll list the users attached to the customer.
...
You can accomplish the other CRUD operations (update and delete) using the [updateUser](/self-managed/composer/builder/api/schema/mutation/updateUser.mdx) and [deleteUser](/self-managed/composer/builder/api/schema/mutation/deleteUser.mdx) mutations, similar to how you [update or delete customers](#updating-a-customer-through-the-api)
...
You can use the [integrations](/self-managed/composer/builder/api/schema/query/integrations.mdx) query to list integrations.
...
The [createInstance](/self-managed/composer/builder/api/schema/mutation/createInstance.mdx) mutation requires four pieces of information:

- The **ID** of a customer this instance is for. This can be found through the [customers query](/self-managed/composer/builder/api/schema/query/customers.mdx).
...
This query returns some additional data about the instance, including a list of [flows](/self-managed/composer/builder/building-integrations.mdx#flows-in-integrations) and their respective webhook URLs for invoking the flows.
...
- `updateInstance` lets you update multiple properties of an instance (like name, description, config variables, etc.).
  ...
  When you programmatically set config variables using the [updateInstance](/self-managed/composer/builder/api/schema/mutation/updateInstance.mdx) mutation, all config variables must be given a value.
  If you omit a config variable, that config variable will be set to its default value (even if it was previously set).

  If you'd like to set just a subset of config variables while leaving the others alone, use the [updateInstanceConfigVariables](/self-managed/composer/builder/api/schema/mutation/updateInstanceConfigVariables.mdx) mutation, described [below](#updating-config-variables-with-updateinstanceconfigvariables).
  :::
...
Before the [updateInstance](/self-managed/composer/builder/api/schema/mutation/updateInstance.mdx) mutation, let's query the instance for its current config variables.
...
If your config variable is a [connection](/self-managed/composer/builder/building-integrations.mdx#connection-config-variables), which usually represents a username, password, API key, OAuth 2.0 connection, the shape is a bit different. It'll be comprised of a set of `inputs` that have a `name`, `type`, and `value`.
...
[User-level](/self-managed/composer/builder/user-level-configuration.mdx) OAuth 2.0 connections can be imported simiarly using the `updateUserLevelOAuth2Connection`.
Note that your config variable ID will start with `VXN...` instead.
:::

### Deploying an Instance Through the API

An instance needs to be [deployed](/self-managed/composer/builder/instances.mdx#deploying-instances) in order for your new configuration to take effect.
To deploy an instance, use the [deployInstance](/self-managed/composer/builder/api/schema/mutation/deployInstance.mdx) mutation:

<GraphqlExample example="deployInstance" />

## Fetching Step Results Through the API

When you invoke an integration [asynchronously](/self-managed/composer/builder/integration-triggers.mdx#synchronous-and-asynchronous-integrations), an `executionId` is returned:
...
You can use the [executionResult](/self-managed/composer/builder/api/schema/query/executionResult.mdx) along with that `executionId` to fetch the results of that execution.
