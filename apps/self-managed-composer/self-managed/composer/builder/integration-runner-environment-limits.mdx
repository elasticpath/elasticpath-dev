...
**Read More**: [Synchronous Invocations and Redirects](/self-managed/composer/builder/integration-triggers.mdx#synchronous-invocations-and-redirects)

### Synchronous invocation timeouts

A webhook request will time out after 30 seconds.
Webhook requests to [synchronous triggers](/self-managed/composer/builder/integration-triggers.mdx/#synchronous-and-asynchronous-integrations) (triggers that wait until the execution finishes running before responding) must complete their work in under 30 seconds.
