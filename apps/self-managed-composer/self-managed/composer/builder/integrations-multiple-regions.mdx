...
The [prism](/self-managed/composer/builder/cli/prism.mdx) CLI tool interacts with the US Commercial region by default.
...
See docs on [Headless prism Usage](/self-managed/composer/builder/cli/cli-usage.mdx#headless-prism-usage-for-cicd-pipelines).
...
See full documentation [here](/self-managed/composer/builder/integrations.mdx/#renaming-flows-and-config-variables-in-yaml-definitions).
:::
...
Alternatively, you can store a [refresh token](/self-managed/composer/builder/cli/cli-usage.mdx#headless-prism-usage-for-cicd-pipelines) for each region.
...
Because integrations are dependent on components, [build and publish](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#publishing-a-component) your components first, and then update your integrations.
...
You can use that ID along with the [`updateInstance`](/self-managed/composer/builder/api/schema/mutation/updateInstance.mdx) GraphQL mutation to update deployed instances to the latest published version of your integration.
