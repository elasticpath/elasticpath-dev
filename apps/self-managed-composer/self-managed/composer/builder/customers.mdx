...
Your [organization](/self-managed/composer/builder/organization.mdx) creates customers and [integrations](/self-managed/composer/builder/integrations.mdx) in Builder.
Then, your customers can either enable instances of your integrations themselves through your [integration marketplace](/self-managed/composer/builder/integration-marketplace.mdx), or your team members can deploy [instances](/self-managed/composer/builder/instances.mdx) to them.
You can create [users](/self-managed/composer/builder/users.mdx#customer-users) for your customers and assign them [roles](/self-managed/composer/builder/users.mdx#customer-user-roles) so that they can manage and have insight into the instances that are deployed to them.
...
Like all Builder resources, customers can be managed via Builder's CLI tool, [prism](/self-managed/composer/builder/cli/cli-usage.mdx), or through the web app by clicking the **Customers** link on the left-hand sidebar.
...
Create a new customer using the [createCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) mutation:
...
Query [customers](/self-managed/composer/builder/api/schema/query/customers.mdx) for a list of customers:
...
Use the [updateCustomer](/self-managed/composer/builder/api/schema/mutation/updateCustomer.mdx) mutation to update a customer:
...
For more information on attaching config variables, see the [instances](/self-managed/composer/builder/instances.mdx) page.
...
Use the [createInstance](/self-managed/composer/builder/api/schema/mutation/createInstance.mdx) mutation to deploy an instance to a customer:
...
[Instances](/self-managed/composer/builder/instances.mdx)

## Managing alert monitors

**Alert monitors** allow you to send alerts to your team when an [alert trigger](/self-managed/composer/builder/monitoring-and-alerting.mdx#alert-triggers) fires on an instance.
...
To create an alert monitor for an instance, you will need to query [alertTriggers](/self-managed/composer/builder/api/schema/query/alertTriggers.mdx) and select which types of triggers should result in an alert:
...
Then, use the [createAlertMonitor](/self-managed/composer/builder/api/schema/mutation/createAlertMonitor.mdx) mutation to create the alert monitor:
...
**For More Information**:
[Alert Monitors](/self-managed/composer/builder/monitoring-and-alerting.mdx)
...
[Logging from Custom Components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx#logger-object)
...
Depending on the [role](/self-managed/composer/builder/users.mdx#customer-user-roles) you assign to the user, the user might have read or write access to their instances.
...
More information about customer user roles can be found on the [users](/self-managed/composer/builder/users.mdx#customer-user-roles) page.
...
More information about user roles can be found on the [users](/self-managed/composer/builder/users.mdx#customer-user-roles) page.
...
To list all customers query [customers](/self-managed/composer/builder/api/schema/query/customers.mdx), or query [customer](/self-managed/composer/builder/api/schema/query/customer.mdx) for a specific customer's users:
...
To create a new customer user, use the [createCustomerUser](/self-managed/composer/builder/api/schema/mutation/createCustomerUser.mdx) mutation.
...
Delete a user using the [deleteUser](/self-managed/composer/builder/api/schema/mutation/deleteUser.mdx) mutation:
...
**For More Information**:
[Users](/self-managed/composer/builder/users.mdx)
...
Delete a customer using the [deleteCustomer](/self-managed/composer/builder/api/schema/mutation/deleteCustomer.mdx) mutation:
...
External IDs are required if you want to route webhook requests to instances deployed to specific customers using [shared](/self-managed/composer/builder/integration-triggers.mdx#webhook-endpoint-configuration) webhook triggers.
...
In addition at a **name** and **description**, a customer created through the [createCustomer](/self-managed/composer/builder/api/schema/mutation/createCustomer.mdx) mutation can be assigned an `externalId`, so that you can match customers in Builder to customers saved in other systems.

See our [Syncing Customers to Builder](/self-managed/composer/builder/custom-components/quickstarts/syncing-customers-to-builder.mdx) quickstart guide for an example of how to sync customers to Builder using our API.
