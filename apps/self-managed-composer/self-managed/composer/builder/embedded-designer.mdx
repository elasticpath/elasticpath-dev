...
Additional documentation on querying the Builder API as a customer user from the embedded SDK is available on the [Embedded API Requests](/self-managed/composer/builder/embedded-api-requests.mdx) article.
...
It is the same view that you would see as an organization team member if you opened a [customer's logs tab](/self-managed/composer/builder/customers.mdx/#accessing-customer-logs).
...
For information on building custom components, see the [Building Custom Components](/self-managed/composer/builder/custom-components/writing-custom-components.mdx) article or the custom component [getting started guide](/self-managed/composer/builder/getting-started/first-custom-component/set-up-environment.mdx).

:::info Customer users must be able to log in to Builder
In order for a customer user to publish a custom component, you must [create a customer user account](/self-managed/composer/builder/users.mdx/#customer-users) and assign them the "Admin" role.
