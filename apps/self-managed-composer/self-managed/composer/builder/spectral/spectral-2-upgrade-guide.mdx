...
[Component versioning](/self-managed/composer/builder/components.mdx#component-versioning) is handled by the platform, so there's no need to pass in a `version` property in your `component` declaration.
You can remove it.
