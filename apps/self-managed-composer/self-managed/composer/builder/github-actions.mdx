...
Builder's [integration-publisher](https://github.com/marketplace/actions/prismatic-integration-publisher) and [component-publisher](https://github.com/marketplace/actions/prismatic-component-publisher) GitHub Actions use the [prism](/self-managed/composer/builder/cli/cli-usage.mdx) CLI tool to publish integrations and components to Builder.
...
- **PATH_TO_YML** (required): The path to the integration yml file that is to be published.
  See our [docs](/self-managed/composer/builder/integrations.mdx#exporting-an-integration) for more information on how to export an integration.
- **INTEGRATION_ID** (required): The ID of the integration to be published.
  The integration must be [imported _manually_](/self-managed/composer/builder/integrations.mdx#importing-an-integration) once into the production tenant before it can be imported automatically with this action.
