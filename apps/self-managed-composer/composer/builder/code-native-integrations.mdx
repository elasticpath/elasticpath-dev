...
Check out [this article](/composer/builder/getting-started/first-custom-component/set-up-environment.mdx) for a detailed guide on setting up your development environment.
...
The functions work the same as custom trigger functions, which are documented in the [Writing Custom Components](/composer/builder/custom-components/writing-custom-components.mdx#instance-deploy-and-delete-events-for-triggers) article.
...
Just like low-code integrations, code-native integrations include a [config wizard](/composer/builder/configuration-wizard.mdx).
...
Connection definitions in CNI are very similar to [custom component connection definitions](/composer/builder/custom-components/writing-custom-components.mdx#adding-connections), but you use the `connectionConfigVar` function instead of the `connection` function.
...
Data sources are defined in CNI similar to how they are defined in [custom components](/composer/builder/custom-components/writing-custom-components.mdx#writing-data-sources), but you use the `dataSourceConfigVar` function rather than the `dataSource` function.
...
You can build advanced UI elements, like field mappers, into your config wizard using [JSON Forms data sources](/composer/builder/jsonforms.mdx).
...
This can be done with or without a [preprocess flow](/composer/builder/endpoint-configuration.mdx#instance-specific-endpoint-with-a-preprocess-flow), and both methods are described below.

Full documentation on endpoint configuration is available in the [Endpoint Configuration](/composer/builder/endpoint-configuration.mdx) article.
...
Builder provides a number of [existing components](/composer/builder/builder-components/component-catalog.mdx) that you can use in your code-native integrations.
...
You can also write unit tests for your code-native integration, similar to [unit tests for custom components](/composer/builder/custom-components/quickstarts/unit-testing-custom-components.mdx).
...
If your integration includes an OAuth 2.0 connection, you can use the same strategy outlined in the [Unit Testing Custom Components](/composer/builder/custom-components/quickstarts/unit-testing-custom-components.mdx#option-2---pull-the-connection-information-from-an-existing-connection-in-prismatic) guide.
