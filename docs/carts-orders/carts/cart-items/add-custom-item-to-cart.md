...
- [Tax items](/docs/carts-orders/carts/tax-items#the-tax-item-object) may optionally be added with the custom item. Only administrators with [client credentials](/guides/Getting%20Started/authentication/Tokens/client-credential-token) are able to do this. If included, they replace any existing taxes on the custom item.
- A cart can contain a maximum of 100 unique items. Items include products and custom items.
- Custom Cart Items are available through [implicit authentication](/guides/Getting%20Started/authentication/Tokens/implicit-token). Ensure that you always check each order has the correct details for each item, most importantly, price.
:::
