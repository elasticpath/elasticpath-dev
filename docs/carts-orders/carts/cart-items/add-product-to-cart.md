...
Adding a Product to Cart is the most common Cart action. If you want to add any [custom products](/docs/carts-orders/carts/cart-items/add-custom-item-to-cart) or [promotions](/docs/carts-orders/carts/cart-items/add-promotion-to-cart), you need to do that as a separate action.
...
First, you must configure a `custom_inputs` attribute when [creating a new product](/docs/api/pxm/products/create-product) or [updating an existing product](/docs/api/pxm/products/update-product). Once you have defined your custom inputs on a product, you must configure the custom inputs in your orders.
...
The `custom_inputs` attribute is stored in the cart item and must not exceed 1MB. When a cart is [checked out](/docs/carts-orders/checkout), the `custom_inputs` attribute becomes part of the [order](/docs/carts-orders/orders/orders-api/get-an-order).
...
- [Tax items](/docs/carts-orders/carts/tax-items#the-tax-item-object) may optionally be added with the product. Only administrators with [client credentials](/docs/authentication/Tokens/client-credential-token) are able to do this. If included, they replace any existing taxes on the product.
...
:::caution
There are a number of actions that happen to your [inventory](/docs/api/pxm/inventory/inventories-introduction) when checking out and paying for an order. For more information, see the [Inventory](/docs/api/pxm/inventory/inventories-introduction) documentation.
:::
