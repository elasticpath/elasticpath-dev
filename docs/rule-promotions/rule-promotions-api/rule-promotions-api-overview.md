...
| `priority` | `integer` | Optional | Specifies the priority of the promotion. Priorities are represented as integers, with higher numbers indicating higher priority. Each priority value must be unique, so no two promotions can have the same priority number. Expired or inactive promotions can have duplicate priorities, but they must be changed to unique when reactived. To remove the priority, pass the `null` value in the [update a Rule Promotion endpoint](/docs/rule-promotions/rule-promotions-api/rule-promotions-management/update-a-rule-promotion). For example, see [Create a Rule Promotion with Priority](/docs/rule-promotions/rule-promotions-api/cart-rule-promotions/create-a-cart-fixed-discount-rule-promotion#request-example---priority-and-stackable-flags). |
| `stackable` | `boolean` | Optional | Specieid whether multiple promotions can be stacked. Set the `stackable` flag to `true` when multiple promotions are applied to the same cart. If set to `false`, the promotions cannot be stacked with other promotions. By default, it is set to `true`. For example, see [Create a Rule Promotion with Stackable Flag](/docs/rule-promotions/rule-promotions-api/cart-rule-promotions/create-a-cart-fixed-discount-rule-promotion#request-example---priority-and-stackable-flags) |
