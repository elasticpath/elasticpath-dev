...
The [code component](/docs/composer/builder/builder-components/code.mdx) allows you to execute product or industry-specific code within an integration.
This page outlines when and how to use a code component.

## Why use the code component?

You will likely have integration logic that can't be solved using the [standard components](/docs/composer/builder/builder-components/component-catalog.mdx) Builder provides.
The portion of your integrations that are specific to your product or industry can be accomplished using code component steps, or [custom components](/docs/composer/builder/custom-components/writing-custom-components.mdx).
...
Within the integration designer, [add a step](/docs/composer/builder/building-integrations.mdx#adding-steps-to-integrations) to your integration.
...
`context.configVars` provides the [Code Component](/docs/composer/builder/builder-components/code.mdx) with access to all [config variables](/docs/composer/builder/building-integrations.mdx#config-variables-in-integrations), including connections, associated with the integration.
...
[Connections](/docs/composer/builder/connections.mdx) are a special type of config variable.
...
Like a custom component, a code step can save and reference [persisted state](/docs/composer/builder/custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state) at the flow (`instanceState`), cross-flow (`crossFlowState`), integration (`integrationState`) or execution (`executionState`) level.
...
To see an example of returning more complex data structures, and a good example use case for a code component, see the [Using a Code Component to Transform Data](/docs/composer/builder/custom-components/quickstarts/code-component-to-transform-data.mdx) quickstart.
...
To see an example use case for returning binary data from a code component, check out our [Generate a PDF with a Code Component](/docs/composer/builder/custom-components/quickstarts/generating-a-pdf-with-a-code-component.mdx).
...
If you need external dependencies, we strongly recommend using a code step for prototyping, but building a [custom component](/docs/composer/builder/custom-components/writing-custom-components.mdx) for production use.
...
A list of all util type functions is available in the [Spectral SDK docs](/docs/composer/builder/spectral/modules/_util_.md).
