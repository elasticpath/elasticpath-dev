...
After [creating a new integration](/docs/composer/builder/integrations.mdx#creating-a-new-integration) or selecting one from your list of integrations, you will find yourself in the **integration designer**.
...
1. The testing drawer lets you [run tests of your integration](/docs/composer/builder/testing-integrations.mdx), supply your integration with sample payloads, and see results of your integration test runs.
1. The majority of the page is taken up by the integration editor pane.
   ...
   You can also create multiple **flows** - each flow has a [trigger](/docs/composer/builder/integration-triggers.mdx) and series of steps to be executed.
...
Actions, like downloading a file from an [SFTP server](/docs/composer/builder/builder-components/sftp.mdx) or posting a message to [Slack](/docs/composer/builder/builder-components/slack.mdx), are added as **steps** of an integration.
...
The [integration triggers](/docs/composer/builder/integration-triggers.mdx) article details how triggers work, and how to invoke your integration.
...
[Custom Component Action Results](/docs/composer/builder/custom-components/writing-custom-components.mdx#action-results)
...
If you need to enter multiple lines of text for an input value, you can use the [Join Lines](/docs/composer/builder/builder-components/text-manipulation.mdx#join-lines) action to concatenate multiple lines of text into a single string.
...
**For More Information**:
[Custom Component Inputs](/docs/composer/builder/custom-components/writing-custom-components.mdx#adding-inputs)
...
Components are [versioned](/docs/composer/builder/components.mdx#component-versions).
...
**For More Information**:
[Component Versioning](/docs/composer/builder/components.mdx#component-versioning)
...
You can use the [branch](/docs/composer/builder/builder-components/branch.mdx) component to branch based on that error.
...
Each flow has its own unique trigger, and so has its own [webhook URL](/docs/composer/builder/integration-triggers.mdx#webhook-triggers) that you can use to invoke that specific flow.
...
Then, [run tests](/docs/composer/builder/testing-integrations.mdx) just as you would with a single-flow integration.

## Config variables in integrations

When your customer configures and activates an instance of your integration, they'll walk through an instance [configuration wizard](/docs/composer/builder/configuration-wizard.mdx) where they'll enter authenticate with third-party apps, select options from dropdown menus, map fields, toggle boolean values, etc.
...
For information on building a configuration wizard experience for your users, see the [Configuration Wizard](/docs/composer/builder/configuration-wizard.mdx) article.

## Synchronous / asynchronous

Integrations can be triggered synchronously or asynchronously.
For more information, see the [integration triggers](/docs/composer/builder/integration-triggers.mdx#synchronous-and-asynchronous-integrations) article.
...
You can configure your [asynchronously](/docs/composer/builder/integration-triggers.mdx#synchronous-and-asynchronous-integrations)-invoked instances to retry if they fail to run to completion.
...
If an instance has failed more than **Retry Attempts** number of times, the run will be marked as _execution failed_ and relevant [alert monitors](/docs/composer/builder/monitoring-and-alerting.mdx), if configured, will fire.
...
**For More Information**:
[Instance Retry and Replay](/docs/composer/builder/instances.mdx#instance-execution-retry-and-replay)
...
Builder provides the [loop component](/docs/composer/builder/builder-components/loop.mdx) to allow you to loop over an array of items, or you can loop a predetermined number of times.
...
**For More Information**:
[The Loop Component](/docs/composer/builder/builder-components/loop.mdx),
[Looping Over Files Quickstart](/docs/composer/builder/custom-components/quickstarts/looping-over-files.mdx),
[Looping Over a Paginated API](/docs/composer/builder/custom-components/quickstarts/looping-over-a-paginated-api.mdx)
...
You can accomplish that with a combination of two loops (one to loop over pages, and one to loop over records on each page) and a [break loop](/docs/composer/builder/builder-components/loop.mdx#break-loop) action that stops loading pages when there are no more left to load:
...
Please reference [this quickstart](/docs/composer/builder/custom-components/quickstarts/looping-over-a-paginated-api.mdx) for an example of how to loop over a paginated API.
...
The [branch](/docs/composer/builder/builder-components/branch.mdx) component allows you to add branching logic to your integration.
...
**For More Information**:
[The Branch Component](/docs/composer/builder/builder-components/branch.mdx),

### Branching on a value

Adding a [Branch on Value](/docs/composer/builder/builder-components/branch.mdx#branchonvalue) action to your integration allows you to create a set of branches based on the value of some particular variable.
...
**For More Information**:
[Branch on Value Action](/docs/composer/builder/builder-components/branch.mdx#branchonvalue)

### Branching on an expression

The [Branch on Expression](/docs/composer/builder/builder-components/branch.mdx#branchonexpression) action allows you to create branches within your integration based on more complex inputs.
...
The same logic can be represented with a group of conditionals in a [Branch on Expression](/docs/composer/builder/builder-components/branch.mdx#branchonexpression) action:
...
**For More Information**:
[Branch on Expression Action](/docs/composer/builder/builder-components/branch.mdx#branchonexpression)
...
Builder provides the [Select Executed Step Result](/docs/composer/builder/builder-components/branch.mdx#select-executed-step-result) to handle that scenario.
...
You can leverage the [Select Executed Step Result](/docs/composer/builder/builder-components/branch.mdx#select-executed-step-result) action to say "get me the incoming or outgoing invoice - whichever one was executed."
...
You can persist "123" using a [Save Value](/docs/composer/builder/builder-components/persist-data.mdx#flow---save-value) action, and then the next time your integration runs it can use [Get Value](/docs/composer/builder/builder-components/persist-data.mdx#flow---get-value) to know that "123" was the most recently processed record.
...
Data can be persisted between runs using the [Persist Data](/docs/composer/builder/builder-components/persist-data.mdx) component.
...
You can store a key/value pair using the [Save Value](/docs/composer/builder/builder-components/persist-data.mdx#flow---save-value) action, or you can use [Persist Data](/docs/composer/builder/builder-components/persist-data.mdx)'s other actions to append to a persisted list.
If you would like to save a timestamp instead, you can use the [Save Current Time](/docs/composer/builder/builder-components/persist-data.mdx#flow---save-current-time) action to save the current time into a key of your choosing.

Later, in a subsequent run, you can fetch the value you saved using the [Get Value](/docs/composer/builder/builder-components/persist-data.mdx#flow---get-value) action.
If a key is not set, **Get Value** will return `null`.

You can remove data from an array, or remove a key/value pair altogether, using [Persist Data](/docs/composer/builder/builder-components/persist-data.mdx)'s other actions.
...
The Process Data's [DeDuplicate](/docs/composer/builder/builder-components/process-data.mdx#deduplicate) action allows you to pass in such an array objects in **descending order**, along with a unique identifier (like `orderid`).
...
For More Information:
[Persisting Data in Custom Components](/docs/composer/builder/custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state)
...
To interact with a SOAP-based API, you can either use the built-in generic [SOAP component](/docs/composer/builder/builder-components/soap.mdx) to fetch and interpret the WSDL and make requests to the API, or you can write your own custom component leveraging the Builder SDK's [SOAP utility functions](/docs/composer/builder/custom-components/writing-custom-components.mdx#soap-utility-functions).
