...
In order to ensure that the input was properly typed, each step that used that input had to use a utility function, like [`util.types.toNumber()`](/docs/composer/builder/spectral/modules/_util_.md/#const-tonumber), to ensure that the value was cast to the proper type.
