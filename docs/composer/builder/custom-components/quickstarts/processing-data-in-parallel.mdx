...
Once we've fetched our data, we can use the [Collection Tools](/docs/composer/builder/builder-components/collection-tools.mdx#chunks) component's **Chunks** action to split the data into managable chunks.
...
Next, add a [Loop Over Items](/docs/composer/builder/builder-components/loop.mdx#loop-over-items) action to your integration and configure it to loop over the chunks you generated.
...
For information on Composer integration limits, see [Integration Limits](/docs/composer/builder/integration-runner-environment-limits.mdx).
