...
The goal of this quickstart is to examine the [Amazon S3 component](/docs/composer/builder/builder-components/aws-s3.mdx) to get you accustomed to developing more advanced components that have multiple actions, handle credentials through connections, and handle non-string binary data input fields.
...
export default component({
  key: "aws-s3",
  documentationUrl: "https://elasticpath.dev/docs/composer/builder/components/aws-s3/",
  public: true,
  display: {
    label: "Amazon S3",
...
The [Slack component](/docs/composer/builder/builder-components/slack.mdx), for example, will only ever post string messages.
...
If a reference to binary data is passed in (for example, from an [HTTP GET](/docs/composer/builder/builder-components/http.mdx#httpget) action's output), `fileContents` will be an object with `.data` and `.contentType` properties.
