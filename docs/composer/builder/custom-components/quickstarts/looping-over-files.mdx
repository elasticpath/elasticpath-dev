...
Our integration will take advantage of the [loop](/docs/composer/builder/builder-components/loop.mdx) component to process files one by one.
...
This will automatically create a Google Cloud Storage [connection](/docs/composer/builder/connections.mdx) to our config wizard.

We'll want our customers to be able to specify their own bucket, so let's add a [Select Bucket](/docs/composer/builder/builder-components/google-cloud-storage.mdx#select-bucket) data source config variable to our config wizard:
...
Next, we'll use the [Deserialize XML](/docs/composer/builder/builder-components/change-data-format.mdx#deserialize-xml) action to convert the XML string into a JavaScript object whose keys can be referenced by subsequent steps.
...
We can use the [Collection Tools](/docs/composer/builder/builder-components/collection-tools.mdx) **Create Object** action to create a new object for us, referencing the results of the **Deserialize XML** step:
...
Next, we'll use the [HTTP](/docs/composer/builder/builder-components/http.mdx) component's **POST Request** action to send the data we generated.
...
We'll use the [Text Manipulation](/docs/composer/builder/builder-components/text-manipulation.mdx) component's **Find & Replace** action for that, once again referencing the loop's `currentItem`:
...
This integration can be [published](/docs/composer/builder/integrations.mdx#publishing-an-integration), and [instances](/docs/composer/builder/instances.mdx) of this integration can be configured and deployed to customers.
