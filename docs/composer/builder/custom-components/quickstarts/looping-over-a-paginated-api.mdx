...
In this tutorial, we'll look at how to use the [loop component](/docs/composer/builder/builder-components/loop.mdx) to loop over a paginated API.
...
First, we'll create a [Loop N Times](/docs/composer/builder/builder-components/loop.mdx#loop-n-times) loop step to loop over pages.
...
1. A [Persist Data - Get Execution Value](/docs/composer/builder/builder-components/persist-data.mdx#execution---get-value) step will help to track the `_start` value in our API request.
...
2. Next, we'll use the value from the previous step to make a call to JSON Placeholder.
   We'll add an [HTTP - GET Request](/docs/composer/builder/builder-components/http.mdx#get-request) step and fetch `https://jsonplaceholder.typicode.com/posts` with a `_limit` search parameter of `25` and a `_start` search paramater of the value we got:
...
We'll do this by adding a [Branch on Expression](/docs/composer/builder/builder-components/branch.mdx#branch-on-expression) step, and we'll check to see if the "Get Posts" step we invoked returned an empty array:
...
In that case we'll add a [Break Loop](/docs/composer/builder/builder-components/loop.mdx#break-loop) step to get out of our main loop:
...
1. Add a [Loop Over Items](/docs/composer/builder/builder-components/loop.mdx#loop-over-items) action that takes the results from the "Get Posts" step in order to loop over the results:
...
1. We'll use a [Branch on Expression](/docs/composer/builder/builder-components/branch.mdx#branch-on-expression) action to determine if the `currentItem` has `isLast=true` (which indicates if we're looping over the last post):
...
2. If we are on the last post, we'll use a [Persist Data - Save Execution Value](/docs/composer/builder/builder-components/persist-data.mdx#execution---save-value) to save the ID of the last post.
