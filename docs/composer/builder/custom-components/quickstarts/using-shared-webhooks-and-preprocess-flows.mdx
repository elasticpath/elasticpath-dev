...
Read more about webhook endpoint configuration on the [Integration Triggers](/docs/composer/builder/integration-triggers.mdx#webhook-endpoint-configuration) article.
...
To determine which customer to dispatch the webhook request to, we'll need a way to map an Acme ERP `customerId` to the [external ID](/docs/composer/builder/customers.mdx#customer-external-ids) that we use in Composer.
...
Read more about preprocess flows on the [Integration Triggers](/docs/composer/builder/integration-triggers.mdx#the-preprocess-flow) article.
